            ...
            if cfg.toolboxes.eeglab.nogui
                if cfg.env.mode == "hpc" && cfg.toolboxes.eeglab.no_update_check_on_hpc
                    try
                        ...
                        helpers.logmsg(master_log, 'WARNING: setpref failed (%s). Continuing.', me2.message);
                    end
                end
                eeglab('nogui');
            else
...
helpers.apply_shared_epoch_rejection           = @apply_shared_epoch_rejection;
...
function [EEG, info] = apply_shared_epoch_rejection(EEG, reject_cfg)
...
idxEEG = find(strcmpi({EEG.chanlocs.type}, 'EEG'));
if isempty(idxEEG)
    return;
end
