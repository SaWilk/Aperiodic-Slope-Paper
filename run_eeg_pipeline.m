...
prof_env = string(getenv_or_empty('PROOF_PROFILE'));
...
switch cfg.paths.profile
    ...
    case "pc_now"
        ...
        cfg.paths.out_root  = fullfile('K:\Wilken_Arbeitsordner\Preprocessed_data\MATRICS\eeg');

    case "pc_shared"
        cfg.paths.bids_root = fullfile('Z:\pb\KLPSY1\KLPSY1-RTG\MATRICS\raw');
        cfg.paths.out_root  = fullfile('Z:\pb\KLPSY1\KLPSY1-RTG\MATRICS\derivatives\preprocessed_eeg');

    case "hpc_hummel"
        cfg.paths.bids_root = fullfile('/beegfs/u/bbf7366/raw', DEFAULT_BIDS_FOLDER_NAME);
        cfg.paths.out_root  = fullfile('/beegfs/u/bbf7366/derivatives/preprocessed_eeg');
...
bids_env = string(getenv_or_empty('PROOF_BIDS_ROOT'));
out_env  = string(getenv_or_empty('PROOF_OUT_ROOT'));
...
cfg.steps.prep02_triggerfix = struct('run', true, 'overwrite_mode', "");
...
helpers.logmsg(master_log, 'Step06: ref=%s | epoch=[%.1f %.1f]s | baseline=[%d 0]ms | faster_z=%.2f robust=%d',...
    string(cfg.prep06.reference_mode), cfg.prep06.epoch_start_s, cfg.prep06.epoch_end_s, cfg.prep06.base_start_ms, ...
    cfg.prep06.faster_z_thresh, cfg.prep06.faster_use_robust_z);
...
try
    ...
    if cfg.steps.prep02_triggerfix.run
        ...
        step_out = proof_eeg_cf_prep02_triggerfix(subj_id, cfg, paths, helpers);
        if ~step_out.ok
    ...
    if cfg.steps.prep03_untilica.run
        ...
        step_out = proof_eeg_cf_prep03_untilica(subj_id, cfg, paths, helpers);
        if ~step_out.ok
    ...
    if cfg.steps.prep04_ica.run
        ...
        step_out = proof_eeg_cf_prep04_ica(subj_id, cfg, paths, helpers);
        if ~step_out.ok
    ...
    if cfg.steps.prep05_after_ica.run
        ...
        step_out = proof_eeg_cf_prep05_after_ica(subj_id, cfg, paths, helpers);
        if ~step_out.ok
    ...
    if cfg.steps.prep06_epoching.run
        ...
        step_out = proof_eeg_cf_prep06_epoching(subj_id, cfg, paths, helpers);
        if ~step_out.ok
