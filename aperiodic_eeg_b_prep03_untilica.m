...
% Referencing
% ------------------------------------------------------------------------
step_cfg.apply_average_reference      = true;   % true|false
step_cfg.average_ref_exclude_non_eeg  = true;   % only EEG channels define reference
...
%% ========================================================================
%  AVERAGE REFERENCE (EEG ONLY, CONFIGURABLE)
% ========================================================================
...
if isfield(step_cfg,'apply_average_reference') && step_cfg.apply_average_reference
    ...
    if isempty(eeg_idx)
        helpers.logmsg_default('prep03_untilica: WARNING no EEG channels found for average reference.');
        EEG = helpers.append_eeg_comment(EEG, 'prep03_untilica: average reference skipped (no EEG channels)');
    else
        ...
        if step_cfg.average_ref_exclude_non_eeg
            ...
            EEG = helpers.append_eeg_comment(EEG, ...
                sprintf('prep03_untilica: average reference applied (EEG-only, n=%d)', numel(eeg_idx)));
        else
            EEG = pop_reref(EEG, []);
            EEG = helpers.append_eeg_comment(EEG, ...
                sprintf('prep03_untilica: average reference applied (all channels, n=%d)', EEG.nbchan));
        end
    ...
    EEG = helpers.append_eeg_comment(EEG, 'prep03_untilica: average reference skipped (config off)');
    if ~isfield(EEG,'etc') || isempty(EEG.etc); EEG.etc = struct(); end
...
if isfield(cfg,'prep03') && isfield(cfg.prep03,'shared_epoch_rejection')
    ...
    if isstruct(reject_cfg) && isfield(reject_cfg,'enable') && reject_cfg.enable ...
        ...
        helpers.logmsg_default('prep03_untilica: %s | ICA shared rejection %d/%d epochs rejected', ...
